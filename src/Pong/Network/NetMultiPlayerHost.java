...
public class NetMultiPlayerHost extends Canvas implements Runnable {
    ...
    public NetMultiPlayerHost() {
        // constructor
    }

    private NetGameClient socketClient;
    private NetGameServer socketServer;
    public NetworkData networkData = new NetworkData(platformOne, platformTwo, pong);
        ...
        while (true) {
            ...
            while (running) {
                ...
                while (delta >= 1) {
                    ticks++;
                    tick();
                    networkData.update(platformOne, platformTwo, pong);
                    // send data
                    delta -= 1;
                    shoudRender = true;
                }
    ...
    public static void main(String[] args) {
        new NetMultiPlayerHost().start();
    }

    public synchronized NetMultiPlayerHost start() {
        running = true;
        new Thread(this).start();
        if (JOptionPane.showConfirmDialog(this, "server?") == 0){
            socketServer = new NetGameServer(this);
            socketServer.start();
        }

        socketClient = new NetGameClient(this, "localhost");
        socketClient.start();
